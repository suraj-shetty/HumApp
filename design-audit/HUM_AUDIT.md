...
> - **Confirmed no dead UI points at the three screens genuinely blocked on a missing MusicKit capability** (lyrics — screen 34; the Settings → Audio quality picker — screen 39; Library's Liked chip / pinned Liked Songs row — the rest of M-6) — checked, none needed removing. `NowPlayingView.swift`'s bottom action row already leaves the lyrics button out (NP-2/NP-6/NP-7/NP-8's fix), `SettingsView.swift` has no Audio quality row, and `LibraryView.swift`'s chip row never grew Liked. Each was built as if the capability didn't exist in the first place, rather than shipped as a link to nowhere and caught later.
> - **Correction: the debug flag to force screens 04–07's auth/subscription states already existed** — an earlier pass of this status update claimed "no debug flag exists to force these" and was simply wrong; `-HumPreviewAuthState` (`notDetermined` / `requesting` / `denied` / `restricted`) and `-HumPreviewSubscriptionState` (`gap` / `unavailable`) are both real, both wired since `HumApp.swift`'s own doc comment describes them, and both were just verified live rather than taken on faith: `-HumPreviewAuthState notDetermined` renders screen 04's invitation exactly, `denied` renders screen 06 with the recovery card and "Where to look" copy, and `-HumPreviewSubscriptionState gap` renders Home's `catalogUnavailable` state. Screen 07 (the connection-failed toast) stays out of reach on purpose, not for lack of a flag — see the note already recorded against it: `MusicAuthorizationService.request()` is non-throwing, so there is no failure mode in the current protocol for any flag to force.
> - **New scope: Board 03 ("iPad & Apple Watch") read for the first time — everything below §11 is iPhone-only; this board was never inventoried at all.** Correction to a claim made earlier this session: Board 03 was first reported as absent from `designs/Hum-All-Platforms.html`, based on a plain-text grep that found only two mentions of "iPad." It is actually present, embedded as a gzip-compressed blob inside the file's JSON asset map (`mime:"text/html", compressed:true` — the same pattern the other two boards and every image-slot component use). Decompressing it surfaces a full spec: a permanent three-column iPad layout (sidebar / middle column / player column) at 1194×834, a separate no-chrome first-run screen, and five Apple Watch screens. None of it has ever been built — confirmed by grep, the only `horizontalSizeClass`/`verticalSizeClass` usage anywhere in the app is one iPhone landscape check in `NowPlayingView`, so `TARGETED_DEVICE_FAMILY = "1,2"` currently means the iPhone layout stretched, not an iPad-shaped app.
>   - **One real architecture conflict found in the design itself:** the iPad first-run screen draws "Connect Spotify" as an equal-weight second option beside Apple Music — directly contradicting `DEVELOPMENT_PLAN.md`'s own risk register ("no non-MusicKit content source... explicitly forbidden") and the fact that every service protocol in this codebase is Apple-Music-only with no second-provider abstraction. Confirmed not in scope; the board needs its button row redrawn as one primary CTA plus the existing real "Browse what's already on this iPad" skip, not a pair.
>   - **One repeated capability regression:** both the iPad player column and Watch Now Playing draw a heart/love toggle — the exact affordance iPhone already had to retitle to "Add to Library" for lack of a MusicKit love API (DECISIONS M-04). The iPad/Watch board predates that fix and never inherited it.
>   - **Two capabilities flagged as unconfirmed rather than assumed either way**, matching this project's own standard for capability claims: the Playlist view's Edit-mode delete-row and drag-reorder affordances (reordering has a known open defect on iPhone — hangs during playback, six failed attempts, per `PROGRESS.md`; removal-from-playlist has no confirmed MusicKit call anywhere in this codebase's usage), and two sidebar destinations — "Recently added" and a flat "Songs" browse — with no query behind them in `MusicCatalogService`/`MusicLibraryService` today.
>   - **Structural gap list** (nothing built, all net-new): the permanent three-column shell itself; the sidebar (six nav destinations, a "Screens" chip switcher, a live Playlists list, an account footer); six middle-column view states (Listen Now, Grid, Album, Artist, Playlist, Search) each a distinct composition, not an iPhone screen resized; Now Playing and Queue merged into one permanent non-modal right column, replacing both the `.fullScreenCover` and the mini capsule ("do not port it to iPad" is explicit in the board's own spec comment); pointer hover states and real keyboard shortcuts (⌘F, space, ⌥→, ⌘⌥U, a 2px focus ring) — zero `.hoverEffect`/`.keyboardShortcut` exists anywhere in the app; a glass rule where only per-column toolbars get glass and sidebar/player are flatly opaque, which `GlassSurface`/`ChromeGlassContainer` don't support as-is; and a separate no-chrome iPad first-run screen with a real offline-browse path iPhone's `ConnectView` has no equivalent for.
>   - **Feature-parity gap, the other direction:** the board also predates several now-real iPhone capabilities and doesn't draw them — Search's Songs/Albums filter chips, the read-only audio-format badge, per-view loading/error states, in-column buffering/connection-lost/subscription-gap treatments (the column is permanent now, so iPhone's full-screen-cover/sheet approach doesn't fit), a sidebar "New Playlist" affordance, and a secondary-click row context menu carrying the same seven actions iPhone's long-press menu already has.
>   - A revision prompt for the Claude Design tool that authored these boards was written covering all of the above (Fix / Add / Flag / Do-Not structure) — not yet run. Nothing in this bullet has been built; this is the read-and-record step only, the same one every other phase in this report got before code changed.
