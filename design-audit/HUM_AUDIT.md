...
>
> **52 findings, across 25 changes, are fixed since this report was written — plus sixteen new/upgraded screens built (below). Of the twenty screens M-8 originally flagged as designed-but-unbuilt, seventeen are now built or resolved; the three still open (Home-offline, Now Playing-lyrics, Settings → Audio quality) are blocked on a real MusicKit capability this app's service layer doesn't expose, not on effort.**
>
...
~~01 Splash~~ · ~~02–03 Onboarding (×2)~~ · ~~09 Home-loading~~ · 11 Home-offline · ~~15 Search-error~~ · ~~18 Library-sync-error~~ · ~~20 Playlist-loading~~ · ~~22 Artist detail~~ · ~~24 Now Playing-buffering~~ · 25 Track unavailable (the app's own `.gap(canBecomeSubscriber: true)` case already routes to Apple's native offer sheet, the same reasoning as SG-1 — nothing further to build) · ~~29 Full-screen error~~ · ~~31 Track context menu~~ · ~~32 Add to playlist~~ · ~~33 Search-focused/recents~~ · 34 Now Playing-lyrics · 35 AirPlay picker (already answered by the system's own `AVRoutePickerView`, wired since the NP-8 fix — see §8.2's NP-11) · ~~36 Playlist-empty~~ · ~~37 Detail-failed~~ · 39 Settings → Audio quality · ~~41 Settings → Privacy~~ · ~~42 New playlist~~

Genuinely unbuilt now: **11** (needs offline/download tracking this app doesn't have), **34** (no MusicKit lyrics API), **39** (no settable MusicKit quality preference for third-party apps). All three stay blocked on a real capability, not on effort.
